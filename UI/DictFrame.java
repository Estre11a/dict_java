...
public class DictFrame extends JFrame {
    ...
    private void AddTransPanel() {
        ...
        SearchWordTextField.setDocument(new RegexDocument("[a-zA-Z\' ]*"));
    }
    ...
    private void setUpListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                for(int i=0;i<DscrpList.length;i++) {
                    if (DscrpList[i] != null) {
                        ...
                        if(DscrpList[i]!=null) {
                            DscrpLikeButton[i].setText((Client2Server.likedNum(getUserName(), DscrpList[i].getWord(), DscrpList[i].getSrcWeb())));
                        }
                    ...
                    if(DscrpList[i]!=null && collectwords.contains(DscrpList[i])) {
                        Image image;
        ...
        UserButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                onLineUserComboBox.removeAllItems();
                offLineUserComboBox.removeAllItems();
    ...
    private void setTransListener(){
        ...
        LastWordButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                //historyIt= (ListIterator<String>) historyWord.iterator();
        ...
        for(int i = 0; i<DscrpLikeButton.length;i++){
            DscrpLikeButton[i].addActionListener(new ActionListener() {//点赞
                @Override
                public void actionPerformed(ActionEvent e) {
                    int index=-1;
                    for(int i = 0;i<DscrpLikeButton.length;i++){
            ...
            DscrpShareButton[i].addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if(index!=-1&&DscrpList[index].isFound()) {
                        curShare= new WordCard(DscrpList[index]);
                        shareWordCard(curShare);
                    }
                }
            });
        }
    ...
    private void setBookListener(){
        ...
        delWordCardButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {//删除卡片
                ...
                if(index!=-1){
                    if(Client2Server.delUserColeList(getUserName(),collectwords.get(index).getWord())) {
                        wordCardLayout.next(cardContainPanel);
        ...
        shareWordCardButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(index!=-1){
                    curShare=collectwords.get(index);
                    shareWordCard(curShare);
                }
            }
        });
    ...
    private void setUserListener(){
        sendMssgButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if(cardFlag) {//传单词卡
                    if(curWord!=null&&Client2Server.sendCard(getUserName(),targetUser,curShare))
        ...
        MessageButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for(int i=0;i<reciveList.size();){
                    ...
                    for(int j=0;j<reciveList.get(i).length();j++){
                        if(reciveList.get(i).charAt(j)=='@')
                            tmpMssgPanel.append("\n");
                        else
                            tmpMssgPanel.append(String.valueOf(reciveList.get(i).charAt(j)));
                    }
                    i++;
                    rcvMssgPanel.add(tmpMssgPanel);
                }
            }
        });
    ...
    void changeLikeState(int i){
        ...
        if(Client2Server.isLiked(getUserName(),DscrpList[i].getWord(),DscrpList[i].getSrcWeb())) {
            if(DscrpList[i]!=null&&Client2Server.sendDisLike(getUserName(),DscrpList[i].getWord(),DscrpList[i].getSrcWeb())) {
            ...
            if(DscrpList[i]!=null&&Client2Server.sendLike(getUserName(),DscrpList[i].getWord(),DscrpList[i].getSrcWeb())){
                DscrpList[i].setLike(DscrpList[i].getLike() + 1);
                Image image;
    ...
    void updateDscrp(){
        for(int i=0;i<DscrpList.length;i++) {
            if (DscrpList[i] != null) {
                ...
                if(getUserName()==null)
                    DscrpLikeButton[i].setText("0");
                else if(DscrpList[i]!=null) {
                    DscrpLikeButton[i].setText((Client2Server.likedNum(getUserName(), DscrpList[i].getWord(), DscrpList[i].getSrcWeb())));
                }
                DscrpPanel[i].setVisible(DscrpList[i].isVisual());
            ...
            if(getUserName()!=null && DscrpList[i]!=null && Client2Server.isLiked(getUserName(),DscrpList[i].getWord(),DscrpList[i].getSrcWeb())) {
                Image image;
                ImageIcon NotCol = new ImageIcon("icon/Like.png");
                image = NotCol.getImage().getScaledInstance(16, 16, Image.SCALE_DEFAULT);
                NotCol = new ImageIcon(image);
                DscrpLikeButton[i].setIcon(NotCol);

            }else{
                Image image;
                ImageIcon Col = new ImageIcon("icon/DisLike.png");
                image= Col.getImage().getScaledInstance(16, 16, Image.SCALE_DEFAULT);
                Col = new ImageIcon(image);
                DscrpLikeButton[i].setIcon(Col);
            }
